...
export default function ClientDetail({ params }: { params: Promise<{ clientId: string }> }) {
  ...
  if (error || !summary) {
    return (
      <div className="flex flex-col min-h-full items-center justify-center p-8 text-white text-center">
        <div className="text-[16px] text-white/50 mb-4">Client not found</div>
  ...
  return (
    <div className="flex flex-col min-h-full p-4 md:p-8 text-white max-w-[1200px] mx-auto w-full gap-6 pb-24">
      {/* Top Navigation */}
      <button 
      ...
      <div className="w-full bg-white/[0.04] border border-white/[0.07] rounded-[14px] p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col min-w-0">
            <h1 className="text-[18px] text-white/[0.85] font-semibold truncate">
              {summary.client_name || "Unnamed"}
            </h1>
            <div className="text-[12px] text-white/[0.35] truncate">{summary.client_email}</div>
            <div className="text-[11px] text-white/[0.25] mt-1">Linked since {linkedDateStr}</div>
          </div>
        </div>
        <div className="flex items-center">
          ...
          <button className="bg-transparent border border-white/[0.1] text-white/40 rounded-lg px-[18px] py-[9px] text-[13px] hover:bg-white/[0.05] hover:text-white/60 transition-colors ml-2 font-medium">
            Send Alert
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-white/[0.04] border border-white/[0.07] rounded-[14px] p-5 flex flex-col justify-between h-[104px]">
          <div className="text-[11px] uppercase tracking-wider text-white/40">Total Check-ins</div>
          <div className="font-serif text-[32px] text-white/90 leading-none">{summary.total_checkins || 0}</div>
        </div>
        
        {/* Card 2 */}
        <div className="bg-white/[0.04] border border-white/[0.07] rounded-[14px] p-5 flex flex-col justify-between h-[104px]">
          <div className="text-[11px] uppercase tracking-wider text-white/40">Most Frequent</div>
          <div 
        ...
        {/* Card 3 */}
        <div className="bg-white/[0.04] border border-white/[0.07] rounded-[14px] p-5 flex flex-col justify-between h-[104px]">
          <div className="text-[11px] uppercase tracking-wider text-white/40">Avg Intensity</div>
        ...
        {/* Card 4 */}
        <div className="bg-white/[0.04] border border-white/[0.07] rounded-[14px] p-5 flex flex-col justify-between h-[104px]">
          <div className="text-[11px] uppercase tracking-wider text-white/40">Low Mood Days</div>
          <div 
      ...
      <div className="w-full bg-white/[0.04] border border-white/[0.07] rounded-[14px] p-6">
        <h2 className="text-[12px] uppercase tracking-[0.1em] text-white/40 mb-8">14-Day Mood</h2>
        <div className="relative w-full h-[180px]">
          {(summary.fourteen_day_trend || []).length === 0 ? (
            <div className="absolute inset-0 flex items-center justify-center text-[13px] text-white/30">
              ...
              {last14.map((pt, i) => {
                ...
                return (
                  <div key={i} className="group relative flex flex-col items-center flex-1 h-full justify-end">
                    ...
                    {/* X-Axis Label */}
                    <div className="absolute -bottom-6 text-[10px] text-white/25">
                      {dayLabel}
                    </div>

                    {/* Tooltip */}
                    {pt.primary_emotion && (
                      <div className="absolute bottom-full mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 w-max max-w-[150px] bg-[#1f1c19] border border-white/[0.08] rounded-lg p-3 text-center shadow-xl">
                        <div className="text-[10px] text-white/40 mb-1">{pt.date}</div>
                        <div className="text-[13px] font-medium" style={{ color }}>{pt.primary_emotion}</div>
                        {pt.avg_intensity && (
                          <div className="text-[11px] text-white/50 mt-1">Intensity: {pt.avg_intensity.toFixed(1)}</div>
                        )}
                      </div>
                    )}
                  </div>
      ...
      <div className="w-full bg-white/[0.04] border border-white/[0.07] rounded-[14px] flex flex-col overflow-hidden">
        <div className="px-6 py-5 border-b border-white/[0.06]">
          <h2 className="text-[12px] uppercase tracking-[0.1em] text-white/40">Recent Sessions</h2>
        </div>
              ...
              return (
                <div key={sess.id} className="grid grid-cols-1 md:grid-cols-[140px_1fr_100px_2fr] gap-4 p-4 md:px-6 md:py-4 border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors items-center">
                  ...
                  <div className="flex items-center md:flex-col md:items-start md:justify-center">
                    <span className="md:hidden text-[10px] uppercase tracking-wider text-white/20 mr-2 w-16">When:</span>
                    <div className="text-[13px] text-white/80">{dateStr}</div>
                    <div className="text-[11px] text-white/30 ml-2 md:ml-0">{timeStr}</div>
                  </div>

                  {/* Emotion Trail */}
                  <div className="flex items-center">
                    <span className="md:hidden text-[10px] uppercase tracking-wider text-white/20 mr-2 w-16">Mood:</span>
                    <div className="text-[13px] text-white/60 truncate">{trail}</div>
                  </div>

                  {/* Intensity */}
                  <div className="flex items-center">
                    <span className="md:hidden text-[10px] uppercase tracking-wider text-white/20 mr-2 w-16">Level:</span>
                  ...
                  <div className="flex items-start">
                    <span className="md:hidden text-[10px] uppercase tracking-wider text-white/20 mr-2 w-16 mt-0.5">Notes:</span>
                    <div className="text-[12px] text-white/40 italic truncate max-w-full">
      ...
      {pushOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#1f1c19] border border-white/[0.08] rounded-[16px] p-8 max-w-[480px] w-full shadow-2xl relative">
            <button onClick={() => setPushOpen(false)} className="absolute top-4 right-4 p-2 text-white/30 hover:text-white/70 transition-colors rounded-full hover:bg-white/5">
              <X className="w-5 h-5" />
            </button>
            
            <h2 className="text-[18px] font-semibold text-white/90 mb-6">Send Support to {summary.client_name?.split(" ")[0]}</h2>
            
            {/* Tabs */}
            <div className="flex bg-white/[0.04] p-1 rounded-xl mb-6">
              <button 
                onClick={() => setPushTab("write")}
                className={`flex-1 py-2 text-[13px] font-medium rounded-lg transition-colors ${pushTab === "write" ? "bg-white/10 text-white" : "text-white/40 hover:text-white/60"}`}
              >
                Write a message
              </button>
              <button 
                onClick={() => setPushTab("pack")}
                className={`flex-1 py-2 text-[13px] font-medium rounded-lg transition-colors ${pushTab === "pack" ? "bg-white/10 text-white" : "text-white/40 hover:text-white/60"}`}
              >
                From a pack
              </button>
            </div>
            ...
            {pushTab === "write" ? (
              <form onSubmit={handleSendPush}>
                <textarea
                  ...
                  className="w-full h-[120px] resize-none bg-white/[0.05] border border-white/[0.1] focus:border-white/[0.25] rounded-[10px] p-4 text-[13px] text-white/80 outline-none transition-colors mb-6 placeholder:text-white/20"
                />
                <button 
                  type="submit"
              ...
              <div className="flex flex-col gap-3 min-h-[120px] max-h-[240px] overflow-y-auto pr-2">
                {loadingPacks ? (
                  <div className="py-8 text-center text-[13px] text-white/30 animate-pulse">Loading packs...</div>
                ) : packs.length === 0 ? (
                  <div className="py-8 text-center text-[13px] text-white/30">You haven't created any Quote Packs yet.</div>
                ) : (
                  packs.map(pack => (
                    <button key={pack.id} className="flex flex-col text-left p-4 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.05] rounded-xl transition-colors">
                      <div className="text-[14px] font-medium text-white/80 mb-1">{pack.name}</div>
      ...
      {/* TOAST */}
      {toast && (
        <div className="fixed bottom-6 right-6 bg-[#1f1c19] border border-white/[0.08] rounded-[10px] px-[18px] py-[12px] text-[13px] text-white/70 shadow-2xl animate-fade-slide-up z-50">
          {toast}
        </div>
      )}
    </div>
